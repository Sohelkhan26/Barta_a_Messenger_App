...
public class InboxActivity extends AppCompatActivity {
    ...
    private String encryptionKey = "UHu Uhu";

    private void fetchEncryptionKey() {
        Toast.makeText(InboxActivity.this, "INside the Invox" + encryptionKey, Toast.LENGTH_SHORT).show();
        database.getReference().child("Contacts")
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            encryptionKey = snapshot.child("encryptionKey").getValue(String.class);
                            if (encryptionKey == null || encryptionKey.isEmpty()) {
                                Toast.makeText(InboxActivity.this, "Encryption key not found", Toast.LENGTH_SHORT).show();
                            }
                        } else {
                            Toast.makeText(InboxActivity.this, "Contact not found", Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(InboxActivity.this, "Failed to retrieve encryption key", Toast.LENGTH_SHORT).show();
                    }
                });
    }
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    for (DataSnapshot snapshot1 : snapshot.getChildren()) {
                        ...
                        if (encryptionKey != null && !encryptionKey.isEmpty()) {
                            try {
                                // Decrypt the message using the retrieved encryption key
                                decryptedmessage = CryptoHelper.decrypt(encryptionKey, message.getMessage());
                            } catch (Exception e) {
                                decryptedmessage = "Decryption failed"; // Fallback message in case of failure
                            }
                            message.setMessage(decryptedmessage);
                        } else {
                            message.setMessage("Decryption failed: Key not available");
                        }
                        localMessageModel.add(message);
                        updateLocalDatabase(message);
                        chatAdapter.notifyDataSetChanged();
                                            ...
                                            public void onComplete(@NonNull Task<DataSnapshot> task) {
                                                if (task.isSuccessful()) {
                                                    ...
                                                    if (ds.exists()) {
                                                        ...
                                                        try {
                                                            decryptedmessagenotification = CryptoHelper.decrypt(encryptionKey, message.getMessage());
                                                        } catch (Exception e) {
                                                            throw new RuntimeException(e);
                                                        }
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!message.isEmpty()) {
                    if (encryptionKey != null && !encryptionKey.isEmpty()) {
                        try {
                            // Encrypt the message using the retrieved encryption key
                        ...
                        // Create a MessageModel object with the encrypted message
                        MessageModel model = new MessageModel(senderId, encryptedMessage);
                        model.setTimestamp(new Date().getTime());
                        inputMessage.setText(""); // Clear the input field after message is prepared

                        // Generate a unique key for the message in the database
                        String key = database.getReference().child("chats")
                                .child(receiverId)
                                .child(senderId)
                                .push().getKey();
                        ...
                        database.getReference().child("chats")
                                ...
                                .setValue(model).addOnSuccessListener(new OnSuccessListener<Void>() {
                                    @Override
                                    public void onSuccess(Void unused) {
                                        // Store the message in the local database and update UI
                                        model.setMessage(message); // Store the plaintext message for local display
                                        updateLocalDatabase(model);
                                        localMessageModel.add(model);
                                        chatAdapter.notifyDataSetChanged();
                                        chatRecyclerView.scrollToPosition(localMessageModel.size() - 1);

                                        // Update contact's last message and timestamp for receiver
                                        database.getReference().child("Contacts").child(receiverId)
                                                .child(senderId).child("last_message")
                                                .setValue(encryptedMessage);
                                        database.getReference().child("Contacts").child(receiverId)
                                                .child(senderId).child("last_sender_name")
                                                .setValue(senderName); // Set the sender's name
                                        database.getReference().child("Contacts").child(receiverId)
                                                .child(senderId).child("message_time")
                                                .setValue(model.getTimestamp());
                                        database.getReference().child("Contacts").child(receiverId)
                                        ...
                                        // Update contact's last message and timestamp for sender
                                        database.getReference().child("Contacts").child(senderId)
                                                .child(receiverId).child("last_message")
                                                .setValue(encryptedMessage);
                                        database.getReference().child("Contacts").child(senderId)
                        ...
                        Toast.makeText(InboxActivity.this, "Encryption key not available", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
